...
//#endregion
//#region ../node_modules/.pnpm/@vue+compiler-core@3.5.40/node_modules/@vue/compiler-core/dist/compiler-core.esm-bundler.js
/**
...
function onCloseTag(el, end, isImplied = false) {
	...
	if (tokenizer.inXML && (stack$1[0] ? stack$1[0].ns : currentOptions.ns) === 0) tokenizer.inXML = false;
	{
...
var FRAGMENT, TELEPORT, SUSPENSE, KEEP_ALIVE, BASE_TRANSITION, OPEN_BLOCK, CREATE_BLOCK, CREATE_ELEMENT_BLOCK, CREATE_VNODE, CREATE_ELEMENT_VNODE, CREATE_COMMENT, CREATE_TEXT, CREATE_STATIC, RESOLVE_COMPONENT, RESOLVE_DYNAMIC_COMPONENT, RESOLVE_DIRECTIVE, RESOLVE_FILTER, WITH_DIRECTIVES, RENDER_LIST, RENDER_SLOT, CREATE_SLOTS, TO_DISPLAY_STRING, MERGE_PROPS, NORMALIZE_CLASS, NORMALIZE_STYLE, NORMALIZE_PROPS, GUARD_REACTIVE_PROPS, TO_HANDLERS, CAMELIZE, CAPITALIZE, TO_HANDLER_KEY, SET_BLOCK_TRACKING, PUSH_SCOPE_ID, POP_SCOPE_ID, WITH_CTX, UNREF, IS_REF, WITH_MEMO, IS_MEMO_SAME, helperNameMap, Namespaces, NodeTypes, ElementTypes, ConstantTypes, locStub, defaultDelimitersOpen, defaultDelimitersClose, Sequences, Tokenizer, CompilerDeprecationTypes, deprecationData, ErrorCodes$1, errorMessages, isFunctionType, isStaticProperty, isStaticPropertyKey, TS_NODE_TYPES, isStaticExp, nonIdentifierRE, isSimpleIdentifier, validFirstIdentCharRE, validIdentCharRE, whitespaceRE, getExpSource, isMemberExpressionBrowser, isMemberExpressionNode, isMemberExpression, fnExpRE, isFnExpressionBrowser, isFnExpressionNode, isFnExpression, propsHelperSet, forAliasRE, defaultParserOptions, currentOptions, currentRoot, currentInput, currentOpenTag, currentProp, currentAttrValue, currentAttrStartIndex, currentAttrEndIndex, inPre, inVPre, currentVPreBoundary, stack$1, tokenizer, forIteratorRE, stripParensRE, specialTemplateDir, windowsNewlineRE, allowHoistedHelperSet, PURE_ANNOTATION, aliasHelper, prohibitedKeywordRE, stripStringRE, transformExpression, transformIf, transformFor, defaultFallback, trackSlotScopes, trackVForSlotScopes, buildClientSlotFn, directiveImportMap, transformElement, transformSlotOutlet, transformOn$1, transformBind, injectPrefix, transformText, seen$1, transformOnce, transformModel$1, validDivisionCharRE, transformFilter, seen, transformMemo, transformVBindShorthand, BindingTypes, noopDirectiveTransform;
...
Use a v-bind binding combined with a v-on listener that emits update:x event instead.`,
	...
	currentVPreBoundary = null;
	stack$1 = [];
	tokenizer = new Tokenizer(stack$1, {
		onerr: emitError,
		ontext(start, end) {
		...
		onclosetag(start, end) {
			...
			if (!currentOptions.isVoidTag(name)) {
				let found = false;
				for (let i = 0; i < stack$1.length; i++) if (stack$1[i].tag.toLowerCase() === name.toLowerCase()) {
					found = true;
					if (i > 0) emitError(24, stack$1[0].loc.start.offset);
					for (let j = 0; j <= i; j++) onCloseTag(stack$1.shift(), end, j < i);
					break;
				}
		...
		onselfclosingtag(end) {
			...
			endOpenTag(end);
			if (stack$1[0] && stack$1[0].tag === name) onCloseTag(stack$1.shift(), end);
		},
		...
		oncdata(start, end) {
			if ((stack$1[0] ? stack$1[0].ns : currentOptions.ns) !== 0) onText(getSlice(start, end), start, end);
			else emitError(1, start - 9);
		},
		onprocessinginstruction(start) {
			if ((stack$1[0] ? stack$1[0].ns : currentOptions.ns) === 0) emitError(21, start - 1);
		}
	});
	forIteratorRE = /,([^,\}\]]*)(?:,([^,\}\]]*))?$/;
	stripParensRE = /^\(|\)$/g;
...
//#endregion
//#region ../node_modules/.pnpm/@vue+compiler-dom@3.5.40/node_modules/@vue/compiler-dom/dist/compiler-dom.esm-bundler.js
...
var init_compiler_dom_esm_bundler = __esmMin((() => {
	...
	TRANSITION$1 = /* @__PURE__ */ Symbol(`Transition`);
	...
	parserOptions = {
		...
		isBuiltInComponent: (tag) => {
			if (tag === "Transition" || tag === "transition") return TRANSITION$1;
	...
	transformTransition = (node, context) => {
		if (node.type === 1 && node.tagType === 1) {
			if (context.isBuiltInComponent(node.tag) === TRANSITION$1) return () => {
...
var policy, tt, unsafeToTrustedHTML, svgNS, mathmlNS, doc, templateContainer, nodeOps, TRANSITION, ANIMATION, vtcKey, DOMTransitionPropsValidators, TransitionPropsValidators, decorate$1, Transition, callHook, hasExplicitCallback, endId, vShowOriginalDisplay, vShowHidden, vShow, CSS_VAR_TEXT, displayRE, semicolonRE, importantRE, prefixes, prefixCache, xlinkNS, veiKey, optionsModifierRE, optionsModifierEventRE, cachedNow, p, getNow, isNativeOn, patchProp, REMOVAL, defineSSRCustomElement, BaseClass, VueElement, positionMap, newPositionMap, moveCbKey, enterCbKey, decorate, TransitionGroup, getModelAssigner, assignKey, vModelText, vModelCheckbox, vModelRadio, vModelSelect, vModelDynamic, systemModifiers, modifierGuards, withModifiers, keyNames, withKeys, rendererOptions, renderer, enabledHydration, render, hydrate, createApp, createSSRApp, ssrDirectiveInitialized, initDirectivesForSSR;
...
//#endregion
//#region ../node_modules/.pnpm/vue@3.5.40_typescript@6.0.3/node_modules/vue/dist/vue.cjs.js
/**
* vue v3.5.40
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
...
//#endregion
//#region ../node_modules/.pnpm/vue@3.5.40_typescript@6.0.3/node_modules/vue/index.js
