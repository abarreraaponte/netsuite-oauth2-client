...
var init_shared_esm_bundler = __esmMin(() => {
  ...
  isGloballyAllowed = /* @__PURE__ */ makeMap(
  ...
  isKnownMathMLAttr = /* @__PURE__ */ makeMap(
  ...
  for (const stmt of body)
    if (stmt.type === "VariableDeclaration") {
      ...
      for (const decl of stmt.declarations)
        for (const id of extractIdentifiers(decl.id)) onIdent(id);
...
function isForStatement(stmt) {
  return (
    stmt.type === "ForOfStatement" || stmt.type === "ForInStatement" || stmt.type === "ForStatement"
  );
}
...
Use a v-bind binding combined with a v-on listener that emits update:x event instead.`,
  ...
  isStaticProperty = (node) =>
    node && (node.type === "ObjectProperty" || node.type === "ObjectMethod") && !node.computed;
    ...
    onclosetag(start, end) {
      ...
      if (!currentOptions.isVoidTag(name)) {
        ...
        for (let i = 0; i < stack$1.length; i++)
          if (stack$1[i].tag.toLowerCase() === name.toLowerCase()) {
            found = true;
...
var init_compiler_dom_esm_bundler = __esmMin(() => {
  ...
  transformTransition = (node, context) => {
    if (node.type === 1 && node.tagType === 1) {
      if (context.isBuiltInComponent(node.tag) === TRANSITION$1)
        return () => {
...
var init_runtime_dom_esm_bundler = __esmMin(() => {
  ...
  Transition = /* @__PURE__ */ decorate$1((props, { slots }) =>
...
var require_vue_cjs = /* @__PURE__ */ __commonJSMin((exports) => {
  ...
  var compilerDom =
    (init_compiler_dom_esm_bundler(), __toCommonJS(compiler_dom_esm_bundler_exports));
  ...
  function compileToFunction(template, options) {
    ...
    function onError(err, asWarning = false) {
      ...
      const codeFrame =
        err.loc && shared.generateCodeFrame(template, err.loc.start.offset, err.loc.end.offset);
      runtimeDom.warn(
        codeFrame
          ? `${message}
${codeFrame}`
